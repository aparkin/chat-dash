...
class PreviewIdentifier:
    ...
    def create_id(cls, prefix: str = None, previous_id: str = None) -> str:
        ...
        suffix = parts[-1]
        if suffix == 'orig':
            return f"{prefix}_{timestamp}_alt1"
        elif suffix.startswith('alt'):
            try:
                n = int(suffix[3:])  # Extract number after 'alt'
                return f"{prefix}_{timestamp}_alt{n+1}"
            except ValueError:
...
class ServiceContext:
    ...
    def to_dict(self) -> dict:
        ...
        Flattens the structure to match the expected format for system message.
        Data and metadata are merged into a single level dictionary.
        """
        # Start with data fields
        result = {
            **self.data,  # Add all data fields
            'source': self.source  # Add source
        }
        
        # Add metadata fields, prefixed with 'meta_' to avoid conflicts
